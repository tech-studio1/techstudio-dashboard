...
const ProductMultiFileDropzone = ({
  className,
  title,
}: {
  className?: string;
  ...
  const [files, setFiles] = useState<string[]>(form?.getValues("medias") ?? []); // Store URLs of uploaded files
  ...
  const removeFile = (fileUrl: string) => {
    ...
    form.setValue("medias", updatedFiles); // Update form value after removal
  };

  const onDropVariant = (variantIndex: number, fileUrl: string) => {
    // Remove file from general medias
    const updatedFiles = files.filter((f) => f !== fileUrl);
    setFiles(updatedFiles);
    form.setValue("medias", updatedFiles);

    // Add file to the selected variant's medias
    const variantMedias =
      form.getValues(`variants.${variantIndex}.medias`) || [];
    form.setValue(`variants.${variantIndex}.medias`, [
      ...variantMedias,
      fileUrl,
    ]);
  };
      ...
      <div className="mt-4 grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-4">
        {files.length > 0 &&
          files.map((fileUrl, index) => (
            <div key={index}>
              <div className="relative aspect-square drop-shadow mb-2">
                <picture>
                  <img
                    src={fileUrl}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  <DropdownMenuGroup>
                    {form?.getValues("variants")?.map((i, idx) => (
                      <DropdownMenuItem
                        key={idx}
                        onClick={() => onDropVariant(idx, fileUrl)}
                      >
                        {i?.color_name}
                      </DropdownMenuItem>
                    ))}
...
export default ProductMultiFileDropzone;
